...
func create_item_icon(slot: Panel) -> TextureRect:
	...
	# 使用KEEP_ASPECT_CENTERED确保图片居中显示并保持比例
	icon.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	
	# 设置布局模式 - 使用整数值1代替常量
	icon.layout_mode = 1  # 1对应于ANCHORS_AND_OFFSETS模式
	...
	# 确保图标填满整个区域
	# 在Godot 4中，使用expand属性而不是expand_mode
	icon.expand = true
...
func create_count_label(slot: Panel) -> Label:
	...
	# 设置布局模式 - 使用整数值1代替常量
	count_label.layout_mode = 1  # 1对应于ANCHORS_AND_OFFSETS模式
	
	# 将数量标签放在右下角
	count_label.anchor_left = 1
	count_label.anchor_top = 1
	count_label.anchor_right = 1
	count_label.anchor_bottom = 1
	
	# 设置偏移，使标签位于右下角
	count_label.offset_left = -15
	count_label.offset_top = -15
